...
describe Oystercard do
  ...
  context "when touching in" do
    ...
    it "checks if card has at least minimum amount" do
      expect{ oystercard.touch_in(entry_station,entry_zone) }.to raise_error("Balance below minimum of #{Oystercard::MIN_BALANCE}")
    end
  ...
  context "when touching out" do
    ...
    it "the card should deduct the minimum fare from balance" do
      ...
      expect{oystercard.touch_out(exit_station,exit_zone)}.to change{oystercard.balance}.by(-Oystercard::MIN_BALANCE)
    end
    ...
    it "adds the exit station to the journey" do
      ...
      oystercard.touch_in(entry_station,entry_zone)
      oystercard.touch_out(exit_station,exit_zone)
      expect(oystercard.journey_log.journeys[0].exit).to eq("#{exit_station}")
    end
  end
